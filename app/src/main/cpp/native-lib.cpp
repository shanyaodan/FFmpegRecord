...
Java_com_example_pc_ffmpegrecord_FFmpeg_encode(JNIEnv *env, jobject instance,
                                               jbyteArray yuvimage_) {
    jbyte *in = env->GetByteArrayElements(yuvimage_, NULL);
    int ret;
    int enc_got_frame = 0;
    int i = 0;
    ...
    env->ReleaseByteArrayElements(yuvimage_, in, 0);
    return 0;
...
{
JNIEXPORT jint JNICALL
Java_com_example_pc_ffmpegrecord_FFmpeg_initial(JNIEnv *env, jobject instance, jint width,
                                                jint height) {

    const char *out_path = "/storage/emulated/0/wsffmpegcamera.flv";
    yuv_width = width;
    yuv_height = height;
